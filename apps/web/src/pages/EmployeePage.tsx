...
function chipTone(level: number) {
  if (level <= 2) return "ui-fb-chip-low";
  if (level >= 4) return "ui-fb-chip-high";
  return "ui-fb-chip-mid";
}
...
export function EmployeePage() {
  ...
  return (
    <div className="space-y-5">
      <EmployeeSubnav />
      <PageHeader title="Hasil penilaian" sub="Skor, radar budaya kerja, dan umpan balik dari pejabat penilai." />
      {err ? <Banner tone="error">{err}</Banner> : null}
      <Card>
        <div className="text-xs font-medium uppercase tracking-wide text-ink-faint">Periode {periodLabel}</div>
        ...
        <div className="mt-1 text-sm text-ink-muted">Penilai: {dash?.assessorName ?? "—"}</div>
      </Card>
      <Card>
        <h2 className="font-semibold">Profil budaya kerja</h2>
        <p className="ui-sub">Radar skala 0–120 untuk akun pegawai ini.</p>
        <BudayaRadar
          ee={view?.calculations.budayaEksekusiEfektif ?? 0}
          ck={view?.calculations.budayaCaraKerjaBaru ?? 0}
          pu={view?.calculations.budayaPelayananUnggul ?? 0}
        />
        {view ? (
          <ul className="space-y-1 text-sm text-ink-muted">
            <li>
              Eksekusi Efektif: {view.calculations.budayaEksekusiEfektif} ({view.calculations.kategoriEksekusiEfektif})
            </li>
            <li>
              Cara Kerja Baru: {view.calculations.budayaCaraKerjaBaru} ({view.calculations.kategoriCaraKerjaBaru})
            </li>
            <li>
              Pelayanan Unggul: {view.calculations.budayaPelayananUnggul} ({view.calculations.kategoriPelayananUnggul})
            </li>
          </ul>
        ) : (
          <p className="text-sm text-ink-faint">Radar terisi setelah penilaian dikirim.</p>
        )}
      </Card>
            ...
            <li key={h.id}>
              ...
              >
                ...
                <span className="text-ink-muted">
                  {h.calculations.scoreScale120} ({h.calculations.kategoriNilaiPerilaku})
                </span>
              </button>
            </li>
          ))}
          {!hist.length && <li className="text-ink-faint">Belum ada riwayat</li>}
        </ul>
      </Card>
    </div>
  );
...
function NilaiHasilCard({ view }: { view: AssessmentView | null }) {
  ...
  const lv = current && current.level > 0 ? levelMeta(current.level) : undefined;

  function summaryLinks(list: NilaiRow[]) {
    return list.map((r, idx) => (
      <span key={r.code}>
        {idx > 0 ? (list.length === 2 ? " dan " : idx === list.length - 1 ? ", dan " : ", ") : null}
        <button type="button" className="ui-btn-quiet !inline !px-0 !py-0" onClick={() => setTab(r.code)}>
          {r.name}
        </button>
      </span>
    ));
  ...
  return (
    <Card>
      ...
      <p className="ui-sub">Level, skor, dan umpan balik penilai untuk setiap nilai BerAKHLAK.</p>
      {!view ? (
        <p className="mt-4 text-sm text-ink-muted">Hasil per nilai muncul setelah penilaian dikirim.</p>
      ) : current ? (
        <>
          <div className="mt-4 space-y-1 text-sm">
            ...
            {!grow.length && !strong.length ? (
              <p className="text-ink-muted">Seluruh nilai berada pada standar yang diharapkan.</p>
            ) : null}
          </div>
          <div className="ui-tabs mt-4" role="tablist" aria-label="Nilai dasar BerAKHLAK">
            {rows.map((r) => {
          ...
          <div className="mt-4 rounded-xl bg-slate-50 px-3.5 py-3 shadow-[inset_0_0_0_1px_rgb(15_23_42/0.06)]">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {current.level > 0 ? (
                ...
                <span className="ui-fb-chip ui-fb-chip-mid">Belum ada level</span>
              )}
            </div>
            <p className="mt-2 text-sm text-ink-muted">
              Skor 120: <span className="font-medium text-ink">{current.score120 || "—"}</span>
            </p>
            {current.feedbackText ? (
              <p className="mt-2 text-sm leading-relaxed text-ink-muted">{current.feedbackText}</p>
            ) : (
              <p className="mt-2 text-sm text-ink-faint">Penilai tidak menampilkan umpan balik untuk nilai ini.</p>
            )}
          </div>
        </>
      ) : null}
