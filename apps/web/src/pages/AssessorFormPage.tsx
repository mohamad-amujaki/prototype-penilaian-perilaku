...
const emptyScores = Object.fromEntries(NILAI_CODES.map((c) => [c, 0])) as Record<string, number>;
...
export function AssessorFormPage() {
  ...
  const [tab, setTab] = useState<NilaiCode>("BP");
  const [step, setStep] = useState<"nilai" | "ringkasan">("nilai");

  useEffect(() => {
    if (!assignmentId) return;
    void Promise.all([api.master(), api.assessorForm(assignmentId)]).then(([m, f]) => {
      setMaster(m);
  ...
  async function save(action: "draft" | "submit") {
    ...
    try {
      ...
      toast.push(action === "submit" ? "Penilaian dikirim" : "Draf penilaian disimpan");
      nav("/penilai");
    } catch (e) {
  ...
  const current = master.nilaiDasar.find((n) => (ID_TO_CODE[n.id] ?? n.code) === tab);
  const panduan = current ? master.panduan.filter((p) => p.nilaiDasarId === current.id) : [];
  const anchors = current
    ? master.anchors.filter((a) => a.nilaiDasarId === current.id).sort((a, b) => a.level - b.level)
    : [];
  ...
  return (
    <div className="pb-4">
      ...
      <div className="sticky top-[3.25rem] z-10 mb-4">
        <div className="ui-tabs" role="tablist" aria-label="Nilai dasar BerAKHLAK">
          ...
          <button
            ...
            title={allDone ? "Ringkasan penilaian" : "Selesaikan ketujuh nilai dasar dulu"}
          >
            Ringkasan
          </button>
        </div>
      ...
      {step === "ringkasan" ? (
        ...
        <Card>
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <h2 className="font-semibold text-brand">Nilai Dasar ASN - {current.name}</h2>
            </div>
          ...
          {panduan.length ? (
            <ul className="mt-3 flex flex-wrap gap-1.5">
              {panduan.map((p) => (
                <li key={p.id} className="rounded-full bg-brand-light px-2.5 py-1 text-xs text-brand-dark">
                  {p.title}
                </li>
              ))}
            </ul>
              ...
              return (
                <label key={a.id} className="ui-radio">
                  ...
                  <span>
                    ...
                    <span className="mt-1 block text-ink-muted">{a.anchorText}</span>
                  </span>
                </label>
              );
            })}
          ...
          {scores[tab] > 0 && (
            <div className="mt-4">
              <label className="ui-label">Umpan balik perilaku kerja pegawai ({(texts[tab] ?? "").length}/300 Karakter)</label>
              <textarea
              ...
              <label className="mt-2 flex items-center gap-2 text-sm text-ink-muted">
                <input
                  ...
                  onChange={(e) => setShow({ ...show, [tab]: e.target.checked })}
                />
      ...
      <div className="ui-sticky-bar">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-ink-muted">
            {step === "nilai" ? `${current?.name ?? ""} · ${doneCount}/7 dinilai` : `Ringkasan · ${doneCount}/7 dinilai`}
          </p>
          <div className="flex flex-wrap gap-2">
            <Button variant="ghost" onClick={prev} disabled={step === "nilai" && tabIndex === 0}>
              Sebelumnya
            </Button>
            {step === "nilai" ? (
              <Button onClick={next} disabled={scores[tab] < 1}>
                {tabIndex === NILAI_CODES.length - 1 ? (allDone ? "Ke ringkasan" : "Nilai berikutnya") : "Nilai berikutnya"}
              </Button>
            ) : (
              <>
                ...
                <Button onClick={() => void save("submit")} disabled={!allDone || !agree}>
                  Simpan
                </Button>
              </>
            )}
