/**
 * master-text.ts — Teks referensi dan seed data untuk penilaian BerAKHLAK
 *
 * Berisi:
 * 1. PANDUAN: 21 pedoman perilaku (3 per nilai dasar) yang menjadi indikator
 * 2. ANCHORS: 35 deskripsi jangkar BARS (5 per nilai dasar) sebagai acuan level
 * 3. FEEDBACKS: 35 template umpan balik (5 per nilai dasar) yang bisa diedit penilai
 *
 * Semua teks dalam Bahasa Indonesia sesuai pedoman KemenPANRB.
 */

import type { NilaiCode } from "./scoring";

/**
 * 21 Pedoman Perilaku Kerja — 3 indikator untuk setiap nilai dasar BerAKHLAK.
 * Digunakan sebagai referensi penilai saat mengisi formulir.
 */
export const PANDUAN: Array<{
  nilaiDasarId: string;  // ID nilai dasar terkait (nilai_bp, nilai_ak, dll.)
  sequence: number;      // Urutan pedoman dalam satu nilai dasar (1, 2, atau 3)
  title: string;         // Teks pedoman perilaku
}> = [
...
/**
 * 35 Deskripsi Jangkar BARS — 5 deskripsi per nilai dasar.
 * Setiap deskripsi menggambarkan perilaku konkret pada level 1-5.
 * Digunakan sebagai acuan penilai saat menentukan level penilaian.
 *
 * Format: ANCHORS[code][level-1]
 * Contoh: ANCHORS["ND-01-BP"][0] = deskripsi jangkar untuk ND-01-BP level 1 (Kontraproduktif)
 */
...
/**
 * 35 Template Umpan Balik — 5 template per nilai dasar.
 * Umpan balik default yang otomatis terisi berdasarkan level penilaian.
 * Penilai bisa mengedit sesuai kebutuhan sebelum disubmit.
 *
 * Format: FEEDBACKS[code][level-1]
 * Contoh: FEEDBACKS["ND-02-AK"][2] = template umpan balik untuk ND-02-AK level 3 (Sesuai Standar)
 */
